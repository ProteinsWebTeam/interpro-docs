...
organism where it is found, a link to the predicted structure in :ref:`AlphaFold structure prediction page <alphafold1>`, 
which now includes predictions from the Big Fantastic Virus Database (BFVD), 
...
gene, the organism where it is found, a link to the protein structure prediction (:ref:`alphafold1`) and a small protein viewer that highlights 
...
AlphaFoldDB now includes the Big Fantastic Virus Database (BFVD) protein structure predictions, which are generated 
by using ColabFold applied to the viral sequence representatives of the UniRef30 clusters.
...
On a the AlphaFold tab of a protein entry page, below the 3D viewer, the :doc:`protein sequence viewer </protein_viewer>` displays the member database 
signatures and InterPro entries matching the protein. Hovering over a match highlights the corresponding section in the 
predicted structure 3D view. Besides, on top of the protein structure viewer there is a drop-down list that 
allows the user to colour the structure according to the per-residue plDDT score or AlphaFold model confidence (default), the predicted TED domains, 
the representative families or the representative domains (if available).
...
external links to the protein entry in the `Uniprot <https://www.uniprot.org/>`_ and/or `AlphaFold <https://alphafold.ebi.ac.uk/>`_ 
websites (depending on availability); and the possibility to start a 
...
When available, different isoforms of the protein can be selected to compare their structural prediction and InterPro matches 
with the consensus protein sequence. When an isoform is selected, a new :doc:`protein sequence viewer </protein_viewer>` 
corresponding to the selection is displayed, with the corresponding structural prediction coloured track. The link available 
by clicking 'pLDDT' in the :doc:`protein sequence viewer </protein_viewer>` leads to the structural prediction of the specific 
isoform in the `AlphaFold <https://alphafold.ebi.ac.uk/>`_ website.
...
UniParc page
************
When a specific protein is not included in `UniProt <https://www.uniprot.org/>`_, the corresponding `UniParc <https://www.uniprot.org/uniparc/>`_ 
sequence entry with its :doc:`protein sequence viewer </protein_viewer>` will be displayed. A link to the entry's 
history in UniProt is provided at the top right side.

.. figure:: images/browse_pages/uniparc_page.png
    :alt: UniParc page of a protein absent from UniProt with its corresponding InterPro matches. 
    :width: 800px

    Uniparc page for the protein sequence `Q16536 <https://www.ebi.ac.uk/interpro/protein/UniProt/Q16536/#table>`_, 
    corresponding to UniParc entry `UPI0000137068 <https://www.uniprot.org/uniparc/UPI0000137068/entry>`_.
...
On the right-hand side, external links to the proteome page in `UniProt <https://www.uniprot.org>`_ and the genome page 
in `Rfam <https://rfam.org/>`_ are provided, when available.
